...
class MetricsCalculator:
    ...
    def compute_metrics(labels: List[str], preds: List[str]) -> Tuple[float, float, float, float, float, float, float]:
        """计算一系列评估指标：人工召回率、机器召回率、平均召回率、准确率、精确率、召回率和F1分数"""
        human_rec, machine_rec, avg_rec = MetricsCalculator.compute_three_recalls(labels, preds)
